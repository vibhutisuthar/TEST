...
namespace MTP_JAPA.Helpers
{
    public class Common
    {
        ...
        /// <returns></returns>
        public static bool SendEmail(string FromEmailID, string FromEmailName, string ToEmailID, string CCEmailID, string BCCEmailID, string subject, string MailBody, Boolean IsBodyHtml)
        {
            try
            {
                MailMessage myMail = new MailMessage();
                myMail.To.Add(ToEmailID);
                if (CCEmailID != null && CCEmailID != "" && ValidateEmailAddress(CCEmailID))
                {
                    myMail.CC.Add(CCEmailID);
                }

                if(!String.IsNullOrEmpty(BCCEmailID) && ValidateEmailAddress(BCCEmailID))
                {
                    myMail.Bcc.Add(BCCEmailID);
                }

                if (WebConfigurationManager.AppSettings["FromAddressOverridde"].ToString() == "")
                {
                    myMail.From = new MailAddress(FromEmailID, FromEmailName);
                }
                else
                {
                    myMail.From = new MailAddress(WebConfigurationManager.AppSettings["FromAddressOverridde"].ToString());
                }
